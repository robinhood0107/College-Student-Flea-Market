...
html class 작명법은 Tailwind css 작명법을 통해서 작성
- html class명이 지저분해지기는 하는데 뭐하는 css인지는 바로 알 수 있음
...
| **View 작업** | **공통 레이아웃(Header):** 로그인 상태에 따른 버튼 분기(`if user`) 처리가 핵심. | **상품 상세/목록:** 카드형 UI 디자인 및 이미지 슬라이더 구현. |
...
#### **Day 1: 환경 설정 및 기반 다지기 (공동)**
* **목표:** 서버가 켜지고(Hello World), DB가 연결되어야 함.
* **Dev A (서버 & 뷰 골격 및 DB & 모델링): (초기세팅으로 힘듦)**
...
* **Dev A (인증 - Auth):**
    * **Backend:** `config/passport.js` 작성 (LocalStrategy). `controllers/authController.js`에 회원가입(bcrypt 암호화), 로그인 로직 구현.
    * **Frontend:** `views/auth/join.ejs` (유효성 검사 포함), `views/auth/login.ejs`.
    * **Route:** `routes/authRoutes.js`.
* **Dev B (상품 등록 - Upload):**
    * **Backend:** `middlewares/upload.js` 작성 (Multer 설정, 파일명 중복 방지, 확장자 제한).
    * **Frontend:** `views/product/write.ejs` 작성 (`enctype="multipart/form-data"` 필수).
    * **Script:** `public/js/product.js`에 이미지 미리보기(Preview) 기능 구현.

#### **Day 3: 세션/접근제어(A) vs 목록/상세 조회(B)**
* **Dev A (권한 & UI 분기):**
...
* **Dev A (마이페이지 - Personal):**
    * **Backend:** `controllers/userController.js`. 내 정보 조회, **내가 등록한 상품 조회**, **내가 찜한 상품 조회(JOIN likes)** 쿼리 작성.
    * **Frontend:** `views/user/profile.ejs`. 탭(Tab) UI를 이용해 판매 내역과 찜 목록 구분 표시.
* **Dev B (상호작용 - AJAX):**
...
#### **Day 5: 관리자(A) vs 검색/필터(B) -**
* **Dev A (관리자 - Admin):**
    //관리자 기능 어려울 경우에는 제외 가능
    ...
    * **Frontend:** `views/admin/dashboard.ejs`. 테이블 형태로 데이터 표시 및 삭제 버튼 구현.
* **Dev B (검색 - Dynamic SQL):** 
    //검색 기능 어려울 경우에는 제외 가능
    * **Backend:** `productController.js`. 검색어(keyword)와 카테고리(category) 유무에 따라 SQL `WHERE`절을 동적으로 생성하는 로직 구현 (SQL Injection 방지 필수).
    * **Frontend:** `header.ejs`의 검색 Form과 `list.ejs`의 카테고리 링크 연동.
...
* **Dev A:** 시연 영상 시나리오 기획 및 녹화 (상품 등록부터 구매 완료까지).
* **공통:** `README.md` 작성 및 최종 DB 덤프(`schema.sql`) 추출.

---

## 3. 파일별 담당자 체크리스트 (Directory Map)

| 디렉터리 | 파일명 | 담당자 | 설명 |
| :--- | :--- | :--- | :--- |
| `/bin` | `www` | **A** | 서버 실행 엔트리 포인트 |
| `/config` | `db.js` | **B** | DB 연결 설정 |
| | `passport.js` | **A** | 로그인 전략 설정 |
| `/controllers` | `authController.js`, `userController.js`, `adminController.js` | **A** | 인증/회원/관리자 로직 |
| | `productController.js` | **B** | 상품/검색/찜 로직 |
| `/routes` | `authRoutes.js`, `userRoutes.js`, `adminRoutes.js` | **A** | A 담당 라우터 |
| | `productRoutes.js`, `index.js` | **B** | B 담당 라우터 (index는 메인) |
| `/middlewares` | `isAuthenticated.js`, `isAdmin.js` | **A** | 권한 체크 미들웨어 |
| | `upload.js` | **B** | 파일 업로드 미들웨어 |
| `/views/partials` | `header.ejs`, `footer.ejs`, `head.ejs` | **A** | 공통 레이아웃 (중요) |
| `/views/auth` | `login.ejs`, `join.ejs` | **A** | 로그인/가입 뷰 |
| `/views/user` | `profile.ejs` | **A** | 마이페이지 뷰 |
| `/views/admin` | `dashboard.ejs` | **A** | 관리자 뷰 |
| `/views/product` | `list.ejs`, `detail.ejs`, `write.ejs`, `edit.ejs` | **B** | 상품 관련 뷰 |
| `/public/js` | `common.js` | **A** | 공통 스크립트 |
| | `product.js` | **B** | 찜/이미지 미리보기 스크립트 |
...
│   └── uploads/             # 이미지 저장 폴더
├── views/                   # EJS 템플릿 파일
│   ├── partials/            # header.ejs, footer.ejs, head.ejs
│   ├── auth/                # login.ejs, join.ejs
