...
* **Dev A (버그 수정 및 기능 개선):**
    * **[이슈 #24](https://github.com/robinhood0107/College-Student-Flea-Market/issues/24): productController.js 버그 수정**
        - `detail` 함수: `user` 변수 전달 누락 → 추가
        - `list` 함수: `user` 변수 전달 누락, catch 블록 `return` 누락 → 수정
        - `create` 함수: `req.user` 체크 누락, 입력값 검증 누락, catch 블록 `return` 누락 → 수정
        - `update` 함수: 입력값 검증 누락 → 추가
        - `detail` 함수: `seller` null 체크 누락 → 추가
        - `toggleLike` 함수: 상품 존재 확인 누락 → 추가
        - `updateStatus` 함수: 상태 값 유효성 검증 누락 → 추가
    * **[이슈 #23](https://github.com/robinhood0107/College-Student-Flea-Market/issues/23): 회원 탈퇴 기능 추가**
        - 프로필 수정 모달에 탈퇴 버튼 추가 (대학교 필드 아래)
        - 탈퇴 버튼 클릭 시 2번 확인 다이얼로그 표시
        - 회원 탈퇴 처리 후 자동 로그아웃
    * **[이슈 #22](https://github.com/robinhood0107/College-Student-Flea-Market/issues/22): Express.js 응답 핸들러 `return` 문 누락 수정**
        - `getEditPage`와 `update` 함수의 catch 블록에서 에러 응답 앞에 `return` 추가
    * **[이슈 #21](https://github.com/robinhood0107/College-Student-Flea-Market/issues/21): 프로필 이미지 업로드 JavaScript 오류 수정**
        - 프로필 아바타 요소에 `id="profile-avatar"` 추가
    * **[이슈 #20](https://github.com/robinhood0107/College-Student-Flea-Market/issues/20): 상품 가격 데이터 타입 일관성 수정**
        - `create` 함수에서도 `parseInt(price)` 적용
    * **[이슈 #19](https://github.com/robinhood0107/College-Student-Flea-Market/issues/19): 프로필 이미지 클릭 시 변경 기능 추가**
        - 프로필 이미지를 `<label>`로 감싸 클릭 가능하게 변경
        - 숨겨진 파일 input 추가 및 AJAX 업로드 구현
    * **[이슈 #18](https://github.com/robinhood0107/College-Student-Flea-Market/issues/18): 드래그 앤 드롭 기능 오류**
        - `product.js` 파일 선택 다이얼로그 취소 시 TypeError 수정
    * **[이슈 #17](https://github.com/robinhood0107/College-Student-Flea-Market/issues/17): 수정 모드에서 여러 파일 선택 시 기존 이미지 유지 기능**
    * **[이슈 #16](https://github.com/robinhood0107/College-Student-Flea-Market/issues/16): 관리자 페이지에서 판매자 정보가 표시되지 않는 문제 수정**
    * **[이슈 #15](https://github.com/robinhood0107/College-Student-Flea-Market/issues/15): 상품 수정 기능 구현**
    * **[이슈 #14](https://github.com/robinhood0107/College-Student-Flea-Market/issues/14): 상품 상세 페이지 오류(게시물 게시자 정보 버그)**
    * **[이슈 #13](https://github.com/robinhood0107/College-Student-Flea-Market/issues/13): 상품 등록에서 사진 드래그 앤 드롭 기능 유실**
    * **[이슈 #12](https://github.com/robinhood0107/College-Student-Flea-Market/issues/12): 마이페이지 누락 기능**
    * **[이슈 #11](https://github.com/robinhood0107/College-Student-Flea-Market/issues/11): env 설정법 추가**
...
│   ├── productController.js   # 상품 등록·조회·수정·삭제, 상태 변경, 댓글 처리
│   ├── userController.js      # 마이페이지, 프로필 조회·수정
│   └── adminController.js     # 관리자 전용 기능(회원 강제 탈퇴 등)
├── models/                  # DB 쿼리 (SQL)
│   ├── User.js
│   └── Product.js
├── routes/                  # URL 주소 정의
